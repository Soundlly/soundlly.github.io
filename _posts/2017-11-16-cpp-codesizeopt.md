...
- 예를 들면 사운들리 신호처리 라이브러리가 200kb정도인데 TensorFlowLite APIset이 tflite 파일을 제외하고 200kb정도 한다면 적용은 못한다는 얘기
- 따라서 열악한 임베디드 플랫폼에서 외부 라이브러리를 그대로 include해서 사용하는 것이 어려운 일입니다. 보통은 최대한 가볍게 만들기 위해서 **필요한 기능만 직접 구현**하게 됩니다 
...
- 여기서 
  1) 데이터 영역은 전역변수 정적(static) 변수가 올라가는 곳. 프로그램 시작시 할당 되고 끝날때 해제됩니다. 
  2) 힙 영역은 동적변수가 올라가는 곳. 프로그램 실행중에 할당되고 해제됩니다. 
